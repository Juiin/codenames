            ...
            if(!socket.spymaster && TURN.toLowerCase() === socket.team){
                if(!BOARD_ARRAY[data].revealed){
                    ...
                    for(var i in SOCKET_LIST){
						...
						SOCKET_LIST[i].emit("addToChat",socket.username + " chose " + "<span style='color:" + BOARD_ARRAY[data].color +"'>" + BOARD_ARRAY[data].word + "</span>");
                    }
                    if(socket.team === "red"){
